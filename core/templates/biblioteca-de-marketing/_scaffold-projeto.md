...
7. **Bookmarks de navegação** — após os painéis Dataview existirem, o Bibliotecário escreve `<vault>/.obsidian/bookmarks.json` apontando pras 14 áreas em ordem de consumo (Biblioteca → Operacional). Ver `plugin/skills/bibliotecario/SKILL.md` → "Fluxo CRIAR BOOKMARKS DE NAVEGAÇÃO" pro pseudocódigo. Idempotente por presença de path interno conhecido (`<prefix>identidade/_identidade.md`); falha graceful se `jq` e Python ambos ausentes.
