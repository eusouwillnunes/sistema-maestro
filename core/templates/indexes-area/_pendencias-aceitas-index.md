...
> Cada vez que você forçou uma entrega após 3 reprovações do QA, fica registrado em `memorias/pendencias-aceitas/historico.md`. Este painel agrupa por item de checklist pra você ver padrões emergentes — se um item reprova sempre, talvez seja melhor ajustar o critério.
...
FROM ""
WHERE file.folder = this.file.folder + "/pendencias-aceitas"
  AND file.name = "historico"
SORT data DESC
