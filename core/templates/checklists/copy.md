# Checklist: Copy (Copywriter)

- [ ] Contexto de marca carregado (identidade + produto)
- [ ] Briefing definido (formato, público, objetivo)
...
- [ ] Conteúdo produzido pelo especialista
- [ ] QA aprovado
- [ ] Revisor aprovado
...
- [ ] **Tags de domínio presentes?** Frontmatter do artefato tem `tags-dominio` preenchido com ≥1 `tema/*`. Se o tipo é `funil`, `campanha`, `lancamento`, `lead-magnet`, `escada-de-valor` ou `analise-performance`, também tem ≥1 `produto/*` (derivado de `produto:` via slugify). Placeholders `[preencher]` não são válidos.
